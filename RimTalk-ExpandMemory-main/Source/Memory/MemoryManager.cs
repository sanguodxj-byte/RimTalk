...
namespace RimTalk.Memory
{
    ...
    public class MemoryManager : WorldComponent
    {
        ...
        private void CheckDailySummarization()
        {
            ...
            {
                // ⭐ 修复：记录昨天的日期，因为我们是在今天总结昨天的记忆
                // GenDate.DaysPassed 在0点时已经是新一天了，所以总结的是前一天的内容
                int summarizedDay = currentDay - 1;
                
                Log.Message($"[RimTalk Memory] 🌙 Day {currentDay}, Hour {currentHour}: Triggering daily ELS summarization (for Day {summarizedDay})");
