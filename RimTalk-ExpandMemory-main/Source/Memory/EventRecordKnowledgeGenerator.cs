    ...
    public static class EventRecordKnowledgeGenerator
    {
        ...
        public static void ScanRecentPlayLog()
        {
            ...
            try
            {
                ...
                // ✅ 修正：Age 代表"发生了多久"，应直接判断是否 <= 1小时
                var recentEntries = gameHistory.AllEntries
                    .Where(e => e != null && e.Age <= GenDate.TicksPerHour)
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(eventText))
                        {
                            ...
                            if (!exists)
                            {
                                ...
                                // ✅ 修正：creationTick 使用事件发生的绝对时刻
                                var entry = new CommonKnowledgeEntry("事件,历史", eventText)
                                {
                                    importance = importance,
                                    isEnabled = true,
                                    isUserEdited = false,
                                    creationTick = currentTick - logEntry.Age,  // ✅ 修正：记录事件发生的绝对时刻
        ...
        private static string ExtractEventInfo(LogEntry logEntry)
        {
            ...
            try
            {
                ...
                // 添加时间前缀
                // ✅ 修正：Age 即为"Ticks Ago"，直接使用
                int ticksAgo = logEntry.Age;
