...
namespace RimTalk.Memory.UI
{
    ...
    public partial class MainTabWindow_Memory : MainTabWindow
    {
        ...
        private void DrawTopBar(Rect rect)
        {
            ...
            // ⭐ 【修复1.5兼容性】在左侧添加更明显的常识库按钮
            float knowledgeButtonX = innerRect.x + 450f;
            float knowledgeButtonWidth = 100f;
            GUI.color = new Color(0.5f, 1f, 0.7f); // 绿色高亮
            if (Widgets.ButtonText(new Rect(knowledgeButtonX, innerRect.y + 5f, knowledgeButtonWidth, 35f), "常识库"))
            {
            ...
            {
                Rect statsRect = new Rect(knowledgeButtonX + knowledgeButtonWidth + 10f, innerRect.y + 8f, 300f, 30f);
